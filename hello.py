...
def index():
    ...
    if form.validate_on_submit():
        ...
        session['val1'] = form.valeur1.data
        session['val2'] = form.valeur2.data
        return redirect(url_for('index'))
    return render_template('index.html', form=form, result=session.get('result'), val1=session.get('val1'), val2=session.get('val2'))
